...
- Optionally exposes the backlight as a Home Assistant switch entity over
  ...
  either way.
- Optionally serves a web UI for adding your own screens, each backed by a
  shell command you type in — no rebuild needed.
...
qnap-led reads an optional YAML file — **default path `/etc/qnap-led/config.yaml`**,
overridable with `--config /some/other/path.yaml` — and layers CLI flags on
top as overrides: a flag always wins over the file, and the file always
wins over built-in defaults. Neither is required: flags alone work exactly
...
- [`cmd/qnap-led/main.go`](cmd/qnap-led/main.go) — CLI, config loading, screen rotation, button/MQTT wiring
...
This section documents the reverse-engineered A125 serial protocol itself
— useful if you're porting this to a different QNAP model or debugging the
panel directly. It's implemented in [`internal/panel/panel.go`](internal/panel/panel.go).
...
with a mutex around the shared serial connection.
...
- Fan control: **not investigated yet**. Unlike the LCD, there's no
  confirmed mechanism (standard Linux `hwmon`/PWM vs. something behind the
  A125 controller) — needs hardware diagnostics before implementing, to
  avoid risking a wrong write to a fan controller on a NAS with a
  non-redundant `tank` pool.
